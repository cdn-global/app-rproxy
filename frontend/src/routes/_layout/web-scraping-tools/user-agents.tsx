...
const getAuthToken = () => {
  ...
  if (!token) {
    console.warn(
      "No access token found. Using a mock token for demonstration. Please log in for a real application.",
    )
...
async function fetchPaginatedUserAgents(
  skip: number,
  limit: number,
): Promise<UserAgentsPublic> {
  const response = await fetch(
    `${API_BASE_URL}/user-agents/?skip=${skip}&limit=${limit}`,
  )
  if (!response.ok) {
    const errorData = await response
      .json()
      .catch(() => ({ detail: "Failed to fetch user agents" }))
...
async function fetchAllUserAgents(): Promise<UserAgentPublic[]> {
  ...
  if (!response.ok) {
    const errorData = await response
      .json()
      .catch(() => ({ detail: "Failed to fetch all user agents" }))
    throw new Error(errorData.detail)
  }
...
async function updateUserAgentsFromSource(): Promise<UpdateSourceResponse> {
  ...
  const response = await fetch(
    `${API_BASE_URL}/user-agents/update-from-source/`,
    {
      method: "POST",
  ...
  if (!response.ok) {
    const errorData = await response
      .json()
      .catch(() => ({ detail: "Failed to update from source" }))
    throw new Error(errorData.detail)
  }
  return response.json()
}
...
async function createUserAgent(
  ...
  if (!response.ok) {
    const errorData = await response
      .json()
      .catch(() => ({ detail: "Failed to create user agent" }))
    throw new Error(errorData.detail)
  }
  return response.json()
}
...
async function updateUserAgent({
  id,
  data,
}: { id: string; data: UserAgentUpdate }): Promise<UserAgentPublic> {
  const token = getAuthToken()
  ...
  if (!response.ok) {
    const errorData = await response
      .json()
      .catch(() => ({ detail: "Failed to update user agent" }))
    throw new Error(errorData.detail)
  }
  return response.json()
}
...
async function deleteUserAgent(id: string): Promise<void> {
  ...
  if (!response.ok && response.status !== 204) {
    const errorData = await response
      .json()
      .catch(() => ({ detail: "Failed to delete user agent" }))
    throw new Error(errorData.detail)
  }
}
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="xl">
      ...
      <ModalContent as="form" onSubmit={handleSubmit}>
        <ModalHeader>
          {initialData ? "Edit User Agent" : "Add New User Agent"}
        </ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <FormControl isRequired>
            ...
            <Textarea
              defaultValue={initialData?.user_agent}
    ...
    >
      <AlertDialogOverlay>
        <AlertDialogContent>
          ...
          <AlertDialogBody>
            Are you sure you want to delete this user agent? This action cannot
            be undone.
          </AlertDialogBody>
          <AlertDialogFooter>
  ...
  if (agents.length === 0) {
    return (
      <Flex justify="center" align="center" p={10}>
        <Text color="gray.500">
          No user agents to display in this category on this page.
        </Text>
      </Flex>
    )
  }
        ...
        <Tbody>
          {agents.map((agent) => (
            <Tr key={agent.id} opacity={isPlaceholderData ? 0.5 : 1}>
              <Td maxW="500px" whiteSpace="normal" wordBreak="break-all">
                {agent.user_agent}
              </Td>
              {/* START: UPDATED CELLS */}
              <Td>
                {agent.device ? (
                  <Badge
                    colorScheme={
                      agent.device.toLowerCase() === "desktop"
                        ? "green"
                        : agent.device.toLowerCase() === "mobile"
                          ? "orange"
                          : "gray"
                    }
                  >
                  ...
                  {isSuperuser && (
                    <>
                      ...
                      <IconButton
                        ...
                        colorScheme="red"
                        size="sm"
                        onClick={() => handleOpenDeleteAlert(agent.id)}
                      />
                    </>
                  )}
                </HStack>
...
function UserAgentsPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => handleCRUDSuccess("User agent created."),
    onError: handleMutationError,
  })
  const updateMutation = useMutation({
    mutationFn: updateUserAgent,
    onSuccess: () => handleCRUDSuccess("User agent updated."),
    onError: handleMutationError,
  })
  const deleteMutation = useMutation({
    mutationFn: deleteUserAgent,
    onSuccess: () => handleCRUDSuccess("User agent deleted."),
    onError: handleMutationError,
  })
  ...
  const desktopAgents = useMemo(
    () =>
      allAgents.filter((agent) => agent.device?.toLowerCase() === "desktop"),
    [allAgents],
  )
  const mobileAgents = useMemo(
    () => allAgents.filter((agent) => agent.device?.toLowerCase() === "mobile"),
    [allAgents],
  )
  const otherAgents = useMemo(
    () =>
      allAgents.filter(
        (agent) =>
          !["desktop", "mobile"].includes(agent.device?.toLowerCase() ?? ""),
      ),
    [allAgents],
  )
      ...
      <Container maxW="full" py={9}>
        <Flex align="center" justify="space-between" py={6}>
          ...
          <Text fontSize="lg" color="gray.600">
            A dynamic list of user agents for web scraping
          </Text>
        </Flex>
        ...
        >
          ...
          >
            ...
            <Menu>
              ...
              <MenuList>
                <MenuItem onClick={() => exportMutation.mutate("csv")}>
                  Export as CSV
                </MenuItem>
                <MenuItem onClick={() => exportMutation.mutate("json")}>
                  Export as JSON
                </MenuItem>
              </MenuList>
            </Menu>
          ...
          <Box borderWidth="1px" borderRadius="lg" overflow="hidden">
            ...
            >
              <Text fontSize="sm" color="gray.600">
                Showing <strong>{displayedAgents.length}</strong> results on
                this page
              </Text>
              <HStack>
                <Button
                ...
                <Text fontSize="sm" mx={4} whiteSpace="nowrap">
                  Page {page + 1} of {totalPages || 1}
                </Text>
                <Button
                  onClick={() => setPage((p) => p + 1)}
                  isDisabled={page + 1 >= totalPages || isPlaceholderData}
                >
                  Next
                </Button>
              </HStack>
