...
const ActiveServicesGrid = ({ features }: ActiveServicesGridProps) => {
  if (!features.length) {
    return (
      <Card className="rounded-[24px] border border-slate-200/70 bg-white/80 p-6 text-center backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/70">
        <CardContent className="space-y-3 p-0">
          <CardTitle className="text-lg">No active services yet</CardTitle>
          <CardDescription>
      ...
      {features.map((feature) => {
        ...
        return (
          ...
          >
            ...
            <CardContent className="mt-6 flex items-center justify-between gap-3 pt-0">
              <Button
                asChild
                variant="outline"
